...
class WeatherApp(QWidget):
    ...
    def display_weather(self, data):
        ...
        self.emoji_label.setText(self.get_weather_emoji(weather_id, data))
        self.description_label.setText(f"{description}")
        self.current_time.setText(f"⏱️: {self.get_local_time(data)}   💧: {humidity}%")
    ...
    @classmethod
    def get_weather_emoji(self, weather_id, data):
        time = self.get_local_time(data)
        part = time.split()
        time, state = part[0], part[1]
        ...
        if 200 <= weather_id <= 232:
            ...
            if (hour > 6 and state == 'PM') or (hour < 6 and state == 'AM'):
                return "🌃" 
            else:
                return "☀️" 
